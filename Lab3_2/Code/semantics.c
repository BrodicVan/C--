...
int compare_type(Type dst,Type src)
{
    ...
    */
    ...
    // printf("debug: compare_type: dst:%d src:%d\n",dst->kind,src->kind);
    // UNKNOWN在本代码中看做万能类型
    ...
    switch(nd_tag)
    {
        ...
        */
        ...
        case CompSt:
            ...
            // Stmt -> CompSt
            else if(parent->generation==Stmt0)
            {
    ...
    else if(nd->kid_num==2)
    {
        ...
        switch(kid_0->tag)
        {
            case MINUS:
                ...
                if(!check_r)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型7: 只有基本类型才能进行求负运算\n",kid_1->row);
            ...
            case NOT:
                ...
                if(!check_r)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型7: 只有INT类型才能进行逻辑运算\n",kid_1->row);
                }
                nd->type->kind == BASIC;
                // 默认给予正确的类型
                nd->type->u.basic = INT;
    ...
    else if(nd->kid_num==3)
    {
        ...
        switch(kid_1->tag)
        {
            ...
            case ASSIGNOP:
                ...
                if(!check_l)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型6: 赋值号左边出现一个只有右值的表达式\n",kid_1->row);
                    nd->type->kind == UNKNOWN;
                }
                // 左右两边类型不匹配
                else if(!check_ok)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型5: 赋值号两边的表达式类型不匹配\n",kid_1->row);
                    nd->type->kind == UNKNOWN;
                }
                else
                {
            ...
            case RELOP:
                ...
                if(!(check_l && check_r))
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型7: 只有基本类型才能进行关系运算\n",kid_1->row);
                }
                // 左右两边类型不匹配
                else if(!check_ok)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型7: 关系运算的操作数类型不匹配\n",kid_1->row);
                }
            ...
            case DOT:
                ...
                if(!unknown_l)
                {
                    ...
                    if(!check_l)
                    {
                        fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型13: 对非结构体型变量使用“.”操作符\n",kid_1->row);
                        nd->type->kind = UNKNOWN;
                    }
                    // 结构体没有对应的域
                    else if(!check_ok)
                    {
                        fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型14: 访问结构体中未定义过的域(%s)\n",kid_1->row,kid_2->name);
                        nd->type->kind = UNKNOWN;
                    }
                    else
                    {
            ...
            case LP:
                ...
                if(func_common==NULL)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型11: 函数(%s)在调用时未经定义\n",kid_0->row,kid_0->name);
                    nd->type->kind = UNKNOWN;
                }
                // 函数已定义
                else 
                {
                    ...
                    {
                        fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型9: 函数(%s)调用时实参与形参的数目不匹配\n",kid_0->row,kid_0->name);
                    }
            ...
            default:
                ...
                if(!(check_l && check_r))
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型7: 只有基本类型可以进行算术运算\n",kid_1->row);
                    nd->type->kind = UNKNOWN;
                }
                // 左右两边类型不匹配
                else if(!check_ok)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型7: 算术运算中的操作数类型不匹配\n",kid_1->row);
                    nd->type->kind = UNKNOWN;
    ...
    else if(nd->kid_num==4)
    {
        ...
        if(kid_2->tag==Exp)
        {
            ...
            if(!(kid_0->type->kind==UNKNOWN ||check_l))
            {
                ...
                fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型10: 对非数组型变量使用“[…]”（数组访问）操作符\n",kid_0->row);
                nd->type->kind = UNKNOWN;
            }
            if(!check_r)
            {
                fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型12: 数组访问操作符“[…]”中出现非整数\n",kid_2->row);
            }
        ...
        else
        {
            ...
            if(func_common==NULL)
            {
                fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型2: 函数(%s)在调用时未经定义\n",kid_0->row,kid_0->name);
                nd->type->kind = UNKNOWN;
            }
            // 函数已定义
            else 
            {
                ...
                while(paras!=NULL)
                {
                    ...
                    {
                        fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型9: 函数(%s)调用时实参与形参的类型不匹配\n",kid_0->row,kid_0->name);
                    }
                    count += 1;
                    paras = paras->tail;
                    args = args->tail;
...
void syn_StructSpecifier(struct Node* nd)
{
    ...
    if(nd->kid_num==2)
    {
        ...
        if(common==NULL)
        {
            fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型17: 直接使用未定义过的结构体(%s)来定义变量\n",nd->kids[1]->row,nd->kids[1]->name);
    ...
    if(nd->kid_num==5)
    {
        ...
        if(old_var!=NULL)
        {
            fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型16：结构体名(%s)与前面定义过的结构体或变量的名字重复\n",kid_1->row,struct_name);
        }
        // 该名称已被使用
        if(old_common!=NULL)
        {
            switch (old_common->kind)
            {
                case STRU:
                    ...
                    break;
                case FUNC:
                    // fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型3：变量(%s)与前面定义/声明过的函数名字重复\n",kid_1->row,struct_name);
                    break;
                case FIELD:
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型16：结构体名(%s)与前面定义过的结构体中的域名/自身内部域名重复\n",kid_1->row,struct_name);
                    break;
                default:
                    break;
            }
        ...
        */
        if(old_common==NULL || (old_common!=NULL && old_common->kind!=STRU))
        {
            ...
            // printf("debug: struct DefList_size:%d\n",kid_3->type->size);
    ...
    switch(nd_tag)
    {
        ...
        case Stmt:
            if(kid_num==3)
            {
                ...
                if(nd->kids[0]->tag==RETURN)
                {
                    ...
                    {                        
                        fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型8: 函数返回值类型不正确\n",nd->row);
                    }
                }
            }
            ...
            else if(kid_num>=5)
            {
                ...
                if(!check_ok)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型21: 只有INT类型才能作为if和while语句的条件\n",nd->row);
                }
            }
            break;
        case Exp:
        ...
        case Dec:
            ...
            else if(kid_num==3)
            {
                ...
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型15: 在定义时对域进行初始化\n",kid_1->row);
                }
                // 判断ASSIGNOP是否合法
            ...
            if(nd->id_type==VAR)
            {
                ...
                if(old_var!=NULL)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行,错误类型3：变量(%s)出现重复定义\n",kid_0->row,kid_0->name);
                }
                // 该名称已被使用
                if(old_common!=NULL)
                {
                    ...
                    {
                        case STRU:
                            fail = 1;fprintf(stderr, "<语义错误> 第%d行,错误类型3：变量(%s)与前面定义过的结构体名字重复\n",kid_0->row,kid_0->name);
                            break;
                        case FUNC:
                            // fail = 1;fprintf(stderr, "<语义错误> 第%d行错误类型3：变量(%s)与前面定义/声明过的函数名字重复\n",kid_0->row,kid_0->name);
                            break;
                        case FIELD:
                            fail = 1;fprintf(stderr, "<语义错误> 第%d行,错误类型3：变量(%s)与前面定义过的结构体中的域名字重复\n",kid_0->row,kid_0->name);
                            break;
                        default:
                            break;
                    }
            ...
            else if (nd->id_type==FIELD)
            {
                ...
                if(old_var!=NULL)
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型20：域名(%s)与前面定义过的变量的名字重复\n",kid_0->row,kid_0->name);
                }
                // 该名称已被使用
                if(old_common!=NULL)
                {
                    ...
                    {
                        case STRU:
                            fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型20：域名(%s)与前面定义的结构体名字重复\n",kid_0->row,kid_0->name);
                            break;
                        case FUNC:
                            // fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型3：变量(%s)与前面定义/声明过的函数名字重复\n",kid_0->row,kid_0->name);
                            break;
                        case FIELD:
                            fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型20：不同结构体中域名(%s)重复\n",kid_0->row,kid_0->name);
                            break;
                        default:
                            break;
                    }
                ...
                {
                    fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型15：结构体中域名(%s)重复定义\n",nd->row,nd->name);
                }
            }
            else
            {
        ...
        case FunDec:
            ...
            if(old_func!=NULL)
            {
                fail = 1;fprintf(stderr, "<语义错误> 第%d行，错误类型4: 函数出现重复定义\n",nd->row);
            }
            // 没被定义则插入
            else
            {
